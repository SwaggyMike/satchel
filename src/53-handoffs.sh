...
prune_handoffs() { # prune_handoffs <handoff-dir> → prints count removed
  # Newest first by filename, not by a date= read out of each body: file_handoff
  # derives the name from the same timestamp it writes into that header, and both
  # naming schemes ('<date>.md', '<date>--<machine>.md') sort lexically in date
  # order. Retention is high enough that a sed per file would be felt.
  #
  # Ranking by body also had a latent bug. It emitted "<date>\t<path>" and read
  # it back with IFS=$'\t' — but Bash counts tab as IFS *whitespace*, so a file
  # with no date= produced a leading-tab line that parsed as date=<path>, f=''.
  # The empty-f guard then skipped it: an undated handoff was never pruned and
  # never counted toward retention, so the directory grew past the bound.
  local dir="$1" kept=0 removed=0 f
